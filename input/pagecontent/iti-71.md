...
The transaction is used by an IUA Authorization Client to pass claims to the IUA Authorization Server and to retrieve access token to be used for authorization of the access to write data to and retrieve data from the Swiss EPR.  

Depending on the claims made by the IUA Authorization Client, two different flavors of access tokens are provided by the IUA Authorization Server: 
...
**Role:** Verifies claims and authentication information, retrieves and validates the user consent for the mHealth App to act on behalf of the user and responds a JWT Access Token to the IUA Authorization Client.  
